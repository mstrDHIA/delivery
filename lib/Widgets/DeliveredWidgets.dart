            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  anothertimer(context: context,order: order),
                  SizedBox(height: deviceheight*0.03,),
                  starsrate(order),

                  if(order.review!=null) Column(children:[
                    SizedBox(height: deviceheight*0.03,),
                    //review(order),
                    listitem(context, order)])
      ...
      Padding(
        ...
        child: Container(
          ...
          child: ListTile(
            ...
            title: Text(order.buyer.lastName+" "+order.buyer.firstName+":",
...
timewidget({context,Orders order,OrderProvider orderProvider}){
  double deviceheight = MediaQuery.of(context).size.height;
    double devicewidth = MediaQuery.of(context).size.width;
 return Consumer<OrderProvider>(
    builder: (BuildContext context, value, Widget child) { 
...
timer({context, order}) {
  ...
  starsrate(Orders order){
    List<Widget> stars=List();
   // int stars=order.stars;
    for(int i=0;i<order.stars;i++){
      stars.add(Image.asset("assets/redstar.png",
      scale: 15,));
    }
...
anothertimer({context, Orders order}) {
    double deviceheight = MediaQuery.of(context).size.height;
    double devicewidth = MediaQuery.of(context).size.width;
    Color color;
    if(order.deliveryDuration>order.orderDuration){
      color=Colors.red;
    }
    else{
            color=Colors.green;
          ...
          Container(
            ...
            child: Center(
              child: Container(
                ...
                Center(
                    child: Text(
                        order.deliveryDuration.toString(),
                      style: TextStyle(
                        color: color,
                        fontSize: 20,
                        fontWeight: FontWeight.w700
